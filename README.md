...
| 7 | [07-reconciliation-bailout.md](./07-reconciliation-bailout.md) | **协调剪枝**：改一个组件会不会遍历整棵树？lanes/childLanes 面包屑 + bailout 分步 trace |
| 8 | [08-layout-vs-render.md](./08-layout-vs-render.md) | **重渲染 ≠ 重布局**：子节点变大，父/兄弟怎么跟着变？布局传播三方向 + render(React)/layout(Yoga) 解耦 |

> 阅读建议：1→2→3 建立「数据怎么流」的宏观图；4→5→6 深入「React 协调器内部怎么运转」；7→8 是性能视角（剪枝 + 布局）的应用。
